...
export default {
    ...
    data() {
        return {
            ...
            meteoPorAeroporto: {}, // Key: IATA code (e.g. VIX), Value: normalized weather data
            rankingAirlines: [], // Array of airline ranking metrics
            alertaAproximacao: false,
            logsEventos: [] // Real-time Kafka topic event logs
        };
    },
    computed: {
        listaVoos() {
    ...
    mounted() {
        ...
        window.addEventListener('aviationstack-flights', this.lidarComAviationStack);
    ...
    methods: {
        ...
        lidarComRanking(event) {
            ...
            this.rankingAirlines = updatedList.map((item, index) => ({
                ...item,
                rank: index + 1
            }));
        ...
        lidarComAviationStack(event) {
            const data = event.detail;
            if (!data) return;
            this.adicionarLog('aviationstack-flights', data);
        },
    ...
    beforeUnmount() {
        ...
        window.removeEventListener('aviationstack-flights', this.lidarComAviationStack);
