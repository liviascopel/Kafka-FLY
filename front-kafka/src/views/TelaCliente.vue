...
export default {
    ...
    mounted() {
        ...
        window.addEventListener('pickup-alerts', this.lidarComAlertaPickup);
    ...
    methods: {
        ...
        lidarComAlertaExposicao(event) {
            ...
            console.log('Cliente processando climate-exposure-alert:', data);
        ...
        lidarComAlertaPickup(event) {
            const data = event.detail;
            if (!data || !data.flightIcao) return;

            console.log('Cliente processando pickup-alerts:', data);
    ...
    beforeUnmount() {
        ...
        window.removeEventListener('pickup-alerts', this.lidarComAlertaPickup);
    }
}
</script>
...
<template>
    <div class="container py-4">
        ...
        <!-- Climate Exposure Alert Modal/Popup -->
        <div v-if="mostrarPopupExposicao && alertaExposicaoClimatica" class="modal-backdrop-custom d-flex align-items-center justify-content-center" style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.6); backdrop-filter: blur(5px); z-index: 1050;">
            <div class="card border-0 shadow-lg p-4 text-center animate-pop mx-3" style="max-width: 500px; border-radius: 20px; background: rgba(255, 255, 255, 0.98);">
                <div class="d-flex align-items-center justify-content-center bg-warning bg-opacity-10 rounded-circle mx-auto mb-3" style="width: 70px; height: 70px;">
                    <span class="fs-1">⛈️</span>
                </div>
                <h5 class="fw-bold text-dark mb-2">Aviso Meteorológico Crítico</h5>
                <p class="text-muted small mb-4">
                    Atenção! Devido às condições meteorológicas adversas em <strong>{{ alertaExposicaoClimatica.airportName }}</strong> que coincidem com a janela do seu voo, o <strong>tempo total de voo ou o horário de chegada poderão ser diferentes</strong>.
                </p>
                <div class="bg-light p-2.5 rounded-3 mb-4 font-monospace text-muted small text-start border">
                    <div class="d-flex justify-content-between border-bottom pb-1 mb-1" style="font-size: 0.8rem;">
                        <span>Terminal/Aeroporto:</span>
                        <strong class="text-dark">{{ alertaExposicaoClimatica.airportIata }}</strong>
                    </div>
                    <div class="d-flex justify-content-between" style="font-size: 0.8rem;">
                        <span>Relação Temporal:</span>
                        <strong class="text-dark">{{ alertaExposicaoClimatica.allenRelation }}</strong>
                    </div>
                </div>
                <div class="d-flex justify-content-center">
                    <button class="btn btn-warning text-white fw-bold px-4 rounded-pill shadow-sm" @click="mostrarPopupExposicao = false">
                        Entendi
                    </button>
                </div>
            </div>
        ...
        <!-- Pickup Alert Modal/Popup -->
        <div v-if="mostrarPopupPickup && alertaPickupCorrente" class="modal-backdrop-custom d-flex align-items-center justify-content-center" style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.6); backdrop-filter: blur(5px); z-index: 1051;">
            <div class="card border-0 shadow-lg p-4 text-center animate-pop mx-3" style="max-width: 500px; border-radius: 20px; background: rgba(255, 255, 255, 0.98);">
                <div class="d-flex align-items-center justify-content-center bg-primary bg-opacity-10 rounded-circle mx-auto mb-3" style="width: 70px; height: 70px;">
                    <span class="fs-1">🚗</span>
                </div>
                <h5 class="fw-bold text-dark mb-2">Alerta de Carona (Pickup)</h5>
                <p class="text-muted small mb-4">
                    O voo solicitado, pousará em {{ alertaPickupCorrente.etaMinutes }} minutos, no aeroporto {{ alertaPickupCorrente.arrivalAirportIata }}. O tempo de seu endereço até o aeroporto de destino é {{ alertaPickupCorrente.travelTimeMinutes }} minutos. Vá para o aeroporto em {{ Math.max(0, alertaPickupCorrente.etaMinutes - alertaPickupCorrente.travelTimeMinutes) }} minutos.
                </p>
                <div class="d-flex justify-content-center">
                    <button class="btn btn-primary text-white fw-bold px-4 rounded-pill shadow-sm" @click="mostrarPopupPickup = false">
                        Entendi
                    </button>
                </div>
            </div>
